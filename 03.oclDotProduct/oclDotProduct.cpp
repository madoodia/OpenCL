...
void DotProductHost(std::vector<float> pfData1,
...
int main()
{
  ...
  std::string filepath = "oclDotProduct.cl";

  uint iNumElements = 100000000;
  uint localWorkSize = 256;
  uint globalWorkSize = roundUp(localWorkSize, iNumElements);

  std::vector<float> srcA(4 * iNumElements);
  std::vector<float> srcB(4 * iNumElements);
  ...
  try
  {
    ...
    {
      Timer timer;
      DotProductHost(srcA, srcB, golden);
    }
